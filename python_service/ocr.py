...
# 1️⃣ DPI arttırma (optimize)
def set_image_dpi(img, scale_factor=2):  # 6 → 2 olarak düşürüldü
    height, width = img.shape[:2]
    new_height = int(height * scale_factor)
    new_width = int(width * scale_factor)
    return cv2.resize(img, (new_width, new_height))

# 2️⃣ Güvenli Normalizasyon
def normalize_image(img):
    if img is None or img.size == 0:
        raise ValueError("normalize_image: Görsel yüklenemedi veya bozuk.")
...
def preprocess_image(image_path):
    ...
    if img is None:
        raise ValueError(f"Görsel okunamadı veya bozuk: {image_path}")
...
def ocr_yap(image_path, dil="tur+eng"):
    try:
        start = time.time()
        print("⏳ OCR işlemi başladı...")

        preprocessed_path = preprocess_image(image_path)
        img = cv2.imread(preprocessed_path)
        if img is None:
            raise ValueError(f"Ön işlenmiş görsel okunamadı: {preprocessed_path}")

        config = r"--oem 3 --psm 6"  # Daha kararlı
        raw_text = pytesseract.image_to_string(img, lang=dil, config=config)

        if os.path.exists(preprocessed_path):
            os.remove(preprocessed_path)

        print(f"✅ OCR tamamlandı. Süre: {round(time.time() - start, 2)} saniye")
